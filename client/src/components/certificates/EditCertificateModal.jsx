...
const editSchema = z.object({
  ...
  certificateFile: z
    ...
    .optional()
    .refine((files) => {
      if (!files || files.length === 0) return true; // File upload optional during edit
      const file = files[0];
...
const EditCertificateModal = ({ open, certificate, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (certificate) {
      ...
      setValue('category', certificate.category || 'Course');
      setValue('issueDate', formatDateStr(certificate.issueDate));
      setValue('expiryDate', formatDateStr(certificate.expiryDate));
      setValue('credentialId', certificate.credentialId || '');
      setValue('credentialUrl', certificate.credentialUrl || '');
      setValue('skills', Array.isArray(certificate.skills) ? certificate.skills.join(', ') : '');
    }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await updateCertificate(certificate._id, formData);
      reset();
      setSelectedFile(null);
      onSuccess(res.data);
      onClose();
    } catch (err) {
      setErrorMsg(getErrorMessage(err, 'Failed to update certificate'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-xs">
        ...
        <div>
          ...
          <div
            ...
            className="border border-slate-800 hover:border-slate-700 rounded-xl p-4 text-center cursor-pointer bg-slate-950/40 hover:bg-slate-900/60 transition-all flex flex-col items-center justify-center"
          >
            <input
              type="file"
              ref={fileInputRef}
              ...
              className="hidden"
            />
            <UploadCloud className="w-5 h-5 text-blue-400 mb-1" />
            {selectedFile ? (
              ...
              <p className="text-xs text-slate-400">
                Click to replace existing document ({certificate?.certificateFile?.originalFileName})
              </p>
            )}
          </div>
