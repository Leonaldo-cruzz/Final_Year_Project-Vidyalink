/**
 * Unit tests for AIService - Portfolio Scoring and ATS Resume Evaluation.
 *
 * Tests verify:
 * - AIService request wrapper validates inputs correctly
 * - AIService throws correct ApiError types for invalid payloads
 * - AIService correctly calls the underlying fetch endpoint
 * - evaluateResumeATS validates required fields
 * - evaluateVerifiedStudentResume handles missing resume gracefully
 * - ResumeEvaluation model shape is correct
 */
...
const Portfolio = (await import('../src/models/portfolio.model.js')).default;
...
describe('AIService - evaluateResumeATS() input validation', () => {
  ...
  it('throws ApiError 400 for missing portfolioId', async () => {
    await expect(service.evaluateResumeATS({
      studentId: 's1',
      verificationStatus: 'VERIFIED',
    })).rejects.toMatchObject({ statusCode: 400 });
  ...
  it('throws ApiError 400 for non-VERIFIED status', async () => {
    await expect(service.evaluateResumeATS({
      studentId: 's1',
      portfolioId: 'p1',
...
describe('AIService - evaluateVerifiedStudentResume() integration', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Resume.findOne to return a resume record
    ...
    // Mock ResumeEvaluation.create
    ResumeEvaluation.create.mockResolvedValue({
      _id: 'eval-id-001',
      atsScore: 78.5,
    });

    // Mock fetch for ATS endpoint
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => mockATSResponse,
  ...
  it('attaches savedEvaluationId to returned result', async () => {
    const result = await service.evaluateVerifiedStudentResume('student-abc');
    expect(result.savedEvaluationId).toBe('eval-id-001');
  });

  it('throws ApiError 400 when studentId is missing', async () => {
    await expect(service.evaluateVerifiedStudentResume(null)).rejects.toMatchObject({
      statusCode: 400,
    });
  });
});

describe('ResumeEvaluation Model - Schema Shape', () => {
  it('model is defined and has expected interface', () => {
    expect(ResumeEvaluation).toBeDefined();
    expect(typeof ResumeEvaluation.create).toBe('function');
  });
});
