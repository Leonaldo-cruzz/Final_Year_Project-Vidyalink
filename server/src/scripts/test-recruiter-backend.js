...
async function runTests() {
  ...
  const _student1Profile = await Profile.create({
  ...
  const _student1Project = await Project.create({
  ...
  const _student1Certificate = await Certificate.create({
    userId: studentUser._id,
  ...
  const _student1Resume = await Resume.create({
    userId: studentUser._id,
  ...
  const _student1Github = await GitHubAccount.create({
    userId: studentUser._id,
  ...
  const _student2Profile = await Profile.create({
  ...
  try {
    ...
    assert(candidateData?.resume?.isAvailable === false, 'Resume remains restricted without public portfolio opt-in');
