...
export const isValidScore = (value) => value !== null
  && value !== undefined
  && value !== ''
...
const mapATSScore = (record, sourceRecords) => {
  const score = mapScore(record, 'atsScore', sourceRecords);
  if (!score) return null;

  return {
    ...score,
    matchedKeywords: Array.isArray(record.matchedKeywords)
      ? record.matchedKeywords.slice(0, 100)
      : [],
    missingKeywords: Array.isArray(record.missingKeywords) ? record.missingKeywords.slice(0, 100) : [],
    detectedSkills: Array.isArray(record.detectedSkills)
      ? record.detectedSkills.slice(0, 100)
      : (Array.isArray(record.matchedSkills) ? record.matchedSkills.slice(0, 100) : []),
    matchedSkills: Array.isArray(record.matchedSkills) ? record.matchedSkills.slice(0, 100) : [],
    missingSkills: Array.isArray(record.missingSkills) ? record.missingSkills.slice(0, 100) : [],
    resumeSections: sanitizeBreakdown(record.resumeSections || record.sections || {}),
    recommendations: Array.isArray(record.recommendations)
      ? record.recommendations.slice(0, 50).map((item) => sanitizeValue(item))
      : [],
  };
};
...
const mapSkillGaps = (record, sourceRecords) => {
  ...
  return withMetadata(record, sourceRecords, {
    ...
    matchPercentage: isValidScore(record.matchPercentage) ? Number(record.matchPercentage) : null,
  });
};
...
class AIResultsService {
  async getPortfolioAISummary(studentId, portfolioId) {
    ...
    return {
      ...
      atsScore: mapATSScore(atsEvaluation, atsSources),
