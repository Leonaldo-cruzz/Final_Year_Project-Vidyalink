...
class AIService {
  ...
  /**
   * Forward a validated GitHub payload to the AI GitHub Analytics observation endpoint.
   * @param {object} payload
   * @returns {Promise<object>}
   */
  async analyzeGitHub(payload) {
    if (!payload || typeof payload !== 'object') {
      throw ApiError.badRequest('Invalid payload supplied for GitHub analysis');
    }

    if (!payload.studentId || !payload.portfolioId) {
      throw ApiError.badRequest('studentId and portfolioId are required for GitHub analysis');
    }

    return this.request('/api/v1/evaluation/github/analyze', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  }
  ...
  async buildVerifiedEvaluationPayload(studentId, portfolioId) {
    ...
    const [projects, certificates, resume, githubAccount, githubAnalytics] = await Promise.all([
      ...
      GitHubAnalytics.findOne({ userId: studentId }).sort({ calculatedAt: -1 }).lean(),
    ]);
    ...
    const sanitizedPayload = {
      ...
      github: githubAnalytics ? {
        repositoryCount: githubAnalytics.repositoryCount || 0,
        activeRepositoryCount: githubAnalytics.activeRepositoryCount || 0,
        totalStars: githubAnalytics.totalStars || 0,
        totalForks: githubAnalytics.totalForks || 0,
        languages: githubAnalytics.languages || [],
        commitCount: githubAnalytics.commitCount || 0,
        recentCommitCount: githubAnalytics.recentCommitCount || 0,
        contributionActivity: githubAnalytics.contributionActivity || {},
        readmeCoverage: githubAnalytics.readmeCoverage || 0,
        documentationCoverage: githubAnalytics.documentationCoverage || 0,
        readmePresent: Boolean(githubAnalytics.readmeCoverage >= 50),
        lastActivity: githubAnalytics.recentActivityDate || null,
      } : githubAccount ? {
        username: githubAccount.githubUsername || '',
        publicRepos: githubAccount.publicRepos || 0,
        repositoryCount: githubAccount.publicRepos || 0,
        commitCount: 0,
        contributionActivity: {
          followers: githubAccount.followers || 0,
          following: githubAccount.following || 0,
        },
        languages: [],
        stars: 0,
        forks: 0,
        readmePresent: true,
        lastActivity: githubAccount.lastSyncedAt || null,
      } : {},
