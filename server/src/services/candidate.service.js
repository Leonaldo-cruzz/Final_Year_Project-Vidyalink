...
class CandidateService {
  ...
  async searchCandidates(filters = {}) {
    ...
    const [projects, portfolios, githubAccounts] = await Promise.all([
      ...
      Portfolio.find({ student: { $in: studentIds }, isPublic: true })
        .select('student isPublic verifiedBy verificationHash')
        .lean(),
    ...
    const publicPortfolios = portfolios.filter((portfolio) => (
      portfolio.isPublic === true && portfolio.verifiedBy && portfolio.verificationHash
    ));
    const publicPortfolioIds = publicPortfolios.map((portfolio) => portfolio._id);
    const [evaluations, readinessEvaluations] = publicPortfolioIds.length > 0
      ? await Promise.all([
          PortfolioEvaluation.find({
            studentId: { $in: studentIds },
            portfolioId: { $in: publicPortfolioIds },
            portfolioScore: { $gte: 0, $lte: 100 },
          }).sort({ evaluatedAt: -1, createdAt: -1 }).lean(),
          IndustryReadinessEvaluation.find({
            studentId: { $in: studentIds },
            portfolioId: { $in: publicPortfolioIds },
            industryReadinessScore: { $gte: 0, $lte: 100 },
    ...
    for (const proj of projects) {
      ...
      if (!projectStatsByStudent.has(sId)) {
        projectStatsByStudent.set(sId, { total: 0, verified: 0, pending: 0, rejected: 0 });
      }
      const stats = projectStatsByStudent.get(sId);
      stats.total += 1;
      if (proj.verificationStatus === 'Verified') {
        stats.verified += 1;
      } else if (proj.verificationStatus === 'Rejected') {
        stats.rejected += 1;
      } else {
        stats.pending += 1;
      }
    ...
    const portfolioSet = new Set(publicPortfolios.map((p) => String(p.student)));
    const scoreMap = new Map();
    for (const evaluation of evaluations) {
      const key = String(evaluation.studentId);
      if (!scoreMap.has(key)) scoreMap.set(key, Number(evaluation.portfolioScore));
    }
    const readinessMap = new Map();
    for (const evaluation of readinessEvaluations) {
      const key = String(evaluation.studentId);
      if (!readinessMap.has(key)) readinessMap.set(key, Number(evaluation.industryReadinessScore));
    }
    ...
    const candidates = validProfiles.map((profile) => {
      ...
      const projStats = projectStatsByStudent.get(sId) || { total: 0, verified: 0, pending: 0, rejected: 0 };
      const hasPortfolio = portfolioSet.has(sId);
      ...
      return {
        ...
        pendingProjectCount: projStats.pending,
        rejectedProjectCount: projStats.rejected,
        githubConnected: isGithubConnected,
        portfolioVerified: hasPortfolio,
        portfolioScore: scoreMap.get(sId) ?? null,
        industryReadinessScore: readinessMap.get(sId) ?? null,
    ...
    const filteredCandidates = candidates.filter((candidate) => {
      const hasScore = candidate.portfolioScore !== null && candidate.portfolioScore !== undefined;
      const score = Number(candidate.portfolioScore);
      const scoreMatches = (minPortfolioScore === undefined || (hasScore && Number.isFinite(score) && score >= minPortfolioScore))
        && (maxPortfolioScore === undefined || (hasScore && Number.isFinite(score) && score <= maxPortfolioScore));
      ...
      return scoreMatches && verificationMatches;
    });

    if (sortBy === 'portfolioScore') {
      filteredCandidates.sort((a, b) => (b.portfolioScore ?? -1) - (a.portfolioScore ?? -1));
    }

    const totalPages = Math.ceil(filteredCandidates.length / limit) || 1;
  ...
  async getCandidateDetails(studentId) {
    ...
    const formattedPortfolios = portfolios
      .filter((portfolio) => portfolio.verifiedBy && portfolio.verificationHash)
      .map((portfolio) => ({
